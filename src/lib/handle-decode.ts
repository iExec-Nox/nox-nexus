...
export function decodeHandle(handle: string): HandleInfo | null {
  ...
  const typeCode = parseInt(hex.slice(60, 62), 16); // byte 30
  const chainId = parseInt(hex.slice(52, 60), 16);  // bytes 26-29
  const version = parseInt(hex.slice(62, 64), 16);  // byte 31

  const solidityType = SOLIDITY_TYPES[typeCode] ?? `unknown(${typeCode})`;

  return { solidityType, chainId, version };
}
